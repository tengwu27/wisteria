...
test('routes construction metadata to its structure ledger', () => {
  assert.equal(constructionStructureId({ structureId: 'castle' }), 'castle');
  assert.equal(
    constructionStructureId({ lockScope: 'room:castle/castle-gallery-room' }),
    'castle'
  );
  assert.equal(constructionStructureId({ lockScope: 'legacy-scope' }), 'library');
});
