...
async function ledgerAtPull(pull, structureId) {
  if (!pull?.head?.sha) return { records: [] };
  const ledgerPath = structurePaths(structureId).constructionLedgerPath;
  const file = await github(
    `/repos/${owner}/${repo}/contents/${ledgerPath}?ref=${encodeURIComponent(pull.head.sha)}`
  );
  return JSON.parse(Buffer.from(file.content, 'base64').toString('utf8'));
}

async function syncNotion(entityIds, status, message, pull, metadata) {
  if (!notionToken || !entityIds.length) return;
  const structureId = constructionStructureId(metadata);
  const ledgerPath = structurePaths(structureId).constructionLedgerPath;
  const [trustedLedger, proposedLedger] = await Promise.all([
    readJson(ledgerPath),
    ledgerAtPull(pull, structureId)
  ]);
