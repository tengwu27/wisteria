    ...
    galleryShowsAllProperties(
      {
        type: 'gallery',
        configuration: {
          properties: properties.map(({ property_id }) => ({ property_id }))
        }
      },
      schema
    ),
    ...
    galleryShowsAllProperties(
      {
        ...
        configuration: {
          properties: properties.map((property, index) =>
            index === 0 ? { ...property, visible: false } : property
          )
        }
      },
      schema
    ...
    galleryShowsAllProperties(
      {
        ...
        configuration: {
          properties: properties.map((property, index) => ({
            ...property,
            property_id: `opaque-view-property-${index}`
          }))
        }
      },
      schema
...
test('canonical artwork verification is read-only and detects identity drift', () => {
  const artwork = {
    wisteriaId: 'still-life',
    artKind: 'painting',
    canonicalSha256: 'a'.repeat(64),
    compositionVersion: 1,
    previewOrPrUrl: 'https://example.com/pr/8'
  };
  assert.deepEqual(canonicalArtworkIdentityErrors({ ...artwork }, artwork), []);
  assert.match(
    canonicalArtworkIdentityErrors({ ...artwork, canonicalSha256: 'b'.repeat(64) }, artwork).join(' '),
    /Canonical SHA-256/
  );
});
