import {
  ...
  NOTION_CONFIG_PATH,
  canonicalArtworkIdentityErrors,
  notionRequest,
  propertyModel,
  readJson,
  retrievePage,
...
async function main() {
  if (!token) {
    if (required) throw new Error('NOTION_TOKEN is required to verify the Castle art catalog.');
    console.log('Castle art catalog verification skipped: NOTION_TOKEN is not configured.');
    return;
  }
  ...
  for (const [sceneId, reference] of Object.entries(config.dataSources?.items ?? {})) {
    ...
    if (Object.keys(missing).length) {
      if (!upsertApproved) {
        throw new Error(`${sceneId}: missing Castle art properties: ${Object.keys(missing).join(', ')}.`);
      }
  ...
  for (const record of ledger.records.filter(
    ...
    if (upsertApproved) {
      ...
      if (!record.notionPageId) {
        throw new Error(`${record.wisteriaId}: approved Notion catalog page is missing.`);
      }
      const page = await retrievePage(token, record.notionPageId);
      const entity = propertyModel(page, 'item', record.sceneId);
      const identityErrors = canonicalArtworkIdentityErrors(entity, artwork);
      if (identityErrors.length) {
        throw new Error(`${record.wisteriaId}: ${identityErrors.join(' ')}`);
      }
    }
  }

  console.log(
    `Verified ${Object.keys(config.dataSources?.items ?? {}).length} Castle art catalogs in ${upsertApproved ? 'approved-write' : 'read-only'} mode; ${alteredSources} schemas extended and ${upserts} approved records upserted.`
  );
}
