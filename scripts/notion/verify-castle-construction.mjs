...
const pendingRecords = ledger.records.filter((record) => record.state === 'pending');
if (pendingRecords.length) {
  if (!ledger.activeReservation || ledger.activeReservation.lockScope !== 'room:castle/castle-gallery-room') {
    errors.push('Pending Castle Gallery construction requires its room reservation.');
  }
} else if (ledger.activeReservation) {
  errors.push('A completed Castle Gallery release cannot retain an active room reservation.');
}
...
if (errors.length) {
  ...
  const reservationState = ledger.activeReservation
    ? 'under the active Gallery reservation'
    : 'with the Gallery reservation released';
  console.log(
    `Verified Castle curated composition v${composition.compositionVersion}, ${composition.placements.length} canonical painting, and ${Object.keys(protectedAssets.assets).length} protected assets ${reservationState}.`
  );
}
