...
export function galleryShowsAllProperties(view, schema = {}, wrap = true) {
  ...
  const properties = view.configuration?.properties ?? [];
  ...
  const optionIsVisible = (propertyConfig) => {
    if (!propertyConfig || propertyConfig.visible === false) return false;
    return wrap ? propertyConfig.wrap !== false : propertyConfig.wrap === false;
  };
  const exactMatch = Object.entries(schema).every(([name, property]) => {
    const propertyId = String(property?.id ?? name);
    const propertyConfig = configured.get(propertyId) ?? configured.get(name);
    return optionIsVisible(propertyConfig);
  });
  if (exactMatch) return true;

  // Some Notion view responses expose opaque view-property IDs that differ
  // from the data-source property IDs. The view cannot contain properties from
  // another source, so an exact cardinality match with no hidden/unwrapped
  // entry is the equivalent read-only proof.
  return (
    properties.length === Object.keys(schema).length &&
    properties.every(optionIsVisible)
  );
}
...
export function canonicalArtworkIdentityErrors(entity, artwork) {
  const errors = [];
  if (!entity) return ['The canonical artwork Notion page is missing.'];
  if (entity.wisteriaId !== artwork.wisteriaId) {
    errors.push('Wisteria ID does not match the approved artwork.');
  }
  if (entity.artKind !== normalizeArtKind(artwork.artKind)) {
    errors.push('Art Type does not match the approved artwork.');
  }
  if (entity.canonicalSha256 !== artwork.canonicalSha256) {
    errors.push('Canonical SHA-256 does not match the Git source artwork.');
  }
  if (entity.compositionVersion !== artwork.compositionVersion) {
    errors.push('Composition Version does not match the approved scene.');
  }
  if (entity.previewOrPrUrl !== artwork.previewOrPrUrl) {
    errors.push('Preview/PR URL does not match the active construction PR.');
  }
  return errors;
}
