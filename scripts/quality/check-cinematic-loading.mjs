...
async function validateCastleGallery(route, html, routeCss) {
  ...
  const artworkPublished = Boolean(hotspot);
  if (
    !html.includes('data-castle-gallery') ||
    !html.includes('data-gallery-stack') ||
    !html.includes('data-gallery-fallback') ||
    layerTags.length !== (artworkPublished ? 3 : 2)
  ) {
    report(`${route.label}: publication-aware base/proxy/shell stack and fallback are invalid`, route, false);
  }
  if (artworkPublished && (
  ...
  if (!artworkPublished) {
    try {
      await stat(resolve(route.artworkHtml));
      report(`${route.label}: unpublished artwork inspector route must not be generated`, route, false);
    } catch (error) {
      if (error.code !== 'ENOENT') {
        report(`${route.label}: inspector publication check failed (${error.message})`, route, false);
      }
    }
    if (html.includes('data-gallery-layer="artwork-proxies"')) {
      report(`${route.label}: unpublished artwork proxy entered the Gallery delivery`, route, false);
    }
    return;
  }
